...
<task type="auto">
  <name>Task 1: ADD NEW FUNCTIONALITY to store calendarEventId on booking creation</name>
  <files>apps/cheshire/src/services/booking.ts</files>
  <action>
**CRITICAL: This is NET NEW functionality, not a refactor.** The current codebase creates Google Calendar events but NEVER stores the returned event ID on the Appointment record. Without this change, reschedule and cancel operations cannot sync to Google Calendar because there's no calendarEventId to reference.

**Current broken state** (booking.ts around lines 662-675):
```typescript
...
try {
  ...
  // BUG: calendarEventId is created but NEVER STORED - it's lost forever!
} catch (err) {
  console.error('[Booking] Failed to create calendar event:', err)
}
```

**What's missing:** The code calls `createCalendarEvent()` and gets back an event ID, but then does nothing with it. The Appointment record in the database never receives this ID.

**ADD this critical functionality** - after `createCalendarEvent()` succeeds, immediately persist the event ID:
...
try {
  ...
  // NEW: Store the calendar event ID on the appointment for future updates/deletes
  // This is the critical missing link - without this, no sync is possible
...
**The critical line being ADDED is:**
```typescript
await prisma.appointment.update({
  where: { id: appointment.id },
  data: { calendarEventId },
...
This single line is what enables the entire reschedule/cancel sync workflow. Without it, the Appointment.calendarEventId field remains null forever.
  </action>
  <verify>
...
2. Run: `grep "prisma.appointment.update" apps/cheshire/src/services/booking.ts` - must show at least 1 occurrence with calendarEventId
3. Run: `cd apps/cheshire && npm run build` - should compile without errors
  </verify>
  <done>
NEW prisma.appointment.update() call added after createCalendarEvent() to persist calendarEventId on the Appointment record, enabling future reschedule/cancel sync.
  </done>
</task>
