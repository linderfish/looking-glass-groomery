...
must_haves:
  truths:
    ...
    - "Photo streak updates after photo attachment"
  artifacts:
  ...
  key_links:
    ...
    - from: "apps/telegram-bot/src/handlers/photos.ts"
      ...
      via: "import and call updatePhotoStreak after attachment"
      pattern: "updatePhotoStreak"
---
...
<task type="auto">
  <name>Task 2: Create photo handler with Before/After keyboard and streak update</name>
...
3. Handle callback queries with regex `/^photo:(before|after):(.+)$/`:
   ...
   - **CRITICAL: Call updatePhotoStreak() after successful attachment**
   - Get the returned streak value
   - Call ctx.answerCallbackQuery({ text: 'Saved!' })
   - Call ctx.editMessageText() to confirm with streak: "BEFORE/AFTER photo saved! (Streak: X days)"
...
4. Streak update wired: `grep -n "updatePhotoStreak" apps/telegram-bot/src/handlers/photos.ts`
  </verify>
  <done>Sending a photo to the bot shows Before/After inline keyboard; selecting one saves to database and updates photo streak</done>
</task>
...
5. photos.ts imports and calls updatePhotoStreak after attachPhotoToAppointment
</verification>

<success_criteria>
- Photo upload flow: Telegram photo -> S3 upload -> Before/After selection -> Database save -> Streak update
- No temp file disk bloat (files cleaned up after S3 upload)
- Graceful handling when no current appointment found
- Streak value displayed in confirmation message
</success_criteria>
